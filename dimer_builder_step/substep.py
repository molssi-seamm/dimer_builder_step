...
"""Base class for substeps in the Dimer Builder step."""
...
class Substep(seamm.Node):
    """A base class for substeps in the Dimer Builder subflowchart.

    The top-level DimerBuilder step iterates over the substeps
    of its subflowchart and calls each substep's `run()` method. Each substep
    is responsible for preparing its own input, invoking Dimer Builder,
    ...
    * `version` / `git_revision` - delegated to the plug-in package so that
    ...
    @property
    def version(self):
        """The semantic version of the dimer_builder_step package."""
        return dimer_builder_step.__version__

    @property
    def git_revision(self):
        """The git revision of the dimer_builder_step package."""
    ...
    @property
    def options(self):
        """The Dimer Builder options, delegated to the parent step."""
        return self.parent.options

    @property
    def input_only(self):
        """Whether to write the input only, not run Dimer Builder."""
